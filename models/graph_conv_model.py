...
from models.graph_conv_layer import GraphConvLayer, GraphPoolingLayer
from models.mol_graph import graph_from_mol_tuple
...
class GraphConvModel(pl.LightningModule):
    ...
    def predict(self, batch):
        ...
        molgraph = graph_from_mol_tuple(mols)
        arrayrep = {
        ...
        atom_features = torch.tensor(arrayrep['atom_features'], dtype=torch.float)
        bond_features = torch.tensor(arrayrep['bond_features'], dtype=torch.float)

        regressed = self.forward(arrayrep, atom_features, bond_features)

        return regressed.cpu().detach().numpy()
