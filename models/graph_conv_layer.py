...
class GraphConvLayer(nn.Module):
    ...
    def __init__(
        ...
        self.target_activation = nn.Linear(in_features, out_features, bias=False)
        self.bias = nn.Parameter(torch.Tensor(np.zeros(out_features)))

        self.degree_linear_layer_list = []
        for k in range(len(self.degrees)):
            # add 6 dim because bond features
            self.degree_linear_layer_list.append(nn.Linear(in_features+6, out_features, bias=False))

        self.degree_linear_layer_list = torch.nn.ModuleList(self.degree_linear_layer_list)

        if self.normalize:
            self.batch_norm = torch.nn.BatchNorm1d(out_features)

    def forward(self, array_rep, atom_features, bond_features):
        target_activations = self.target_activation(atom_features)
        ...
        for i, degree in enumerate(self.degrees):
            ...
            if len(atom_neighbors_list) > 0:
                stacked_neighbors_tensor = torch.cat([
                    atom_features[atom_neighbors_list], 
                    bond_features[bond_neighbors_list]
                    ], axis=2)
                summed_neighbors_tensor = torch.sum(stacked_neighbors_tensor, axis=1)
                activations = self.degree_linear_layer_list[i](summed_neighbors_tensor)
                activations_by_degree.append(activations)

        neighbor_activations = torch.cat(activations_by_degree, axis=0)
        total_activations = neighbor_activations + target_activations + self.bias
...
class GraphPoolingLayer(nn.Module):
    ...
    def forward(self, array_rep, atom_features):
        ...
        tmp = self.linear_layer(atom_features)
